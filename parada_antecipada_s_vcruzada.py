...
class MLP:
    ...
    def treinar(self, X_teste, Y_teste, X_restante, Y_restante, epocas=1000, early_stopping_rounds=50):
        acuracias = []
        erros = []
        val_acuracias = []
        val_erros = []
    ...
    def plotar_metricas(self, acuracias, erros, val_acuracias, val_erros, diretorio_saida):
        ...
        plt.legend()
        plt.savefig(os.path.join(diretorio_saida, "grafico_acuracia_fold.png"))
        plt.show()
        ...
        plt.legend()
        plt.savefig(os.path.join(diretorio_saida, "grafico_erro_fold.png"))
        plt.show()
    ...
    def plotar_matriz_confusao(self, Y_verdadeiro, Y_previsto, diretorio_saida):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(diretorio_saida, "matriz_confusao_fold.png"))
        plt.show()
...
def busca_em_grade(X, Y, grid_parametros):
    ...
    for taxa_aprendizado in grid_parametros['taxa_aprendizado']:
        for tamanho_cam_oculta in grid_parametros['tamanhos_cam_oculta']:
            acuracias_fold = []
